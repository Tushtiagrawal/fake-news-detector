...
try:
    ...
    st.error("Vectorizer file not found. Please upload verctorization.jb.")
    st.stop()
except Exception as e:
    st.error(f"Error loading vectorizer: {e}")
    st.stop()
...
try:
    ...
    st.error(f"Error loading one or more models: {e}")
    st.stop()

# --- 3. UI ---
st.title("📰 Fake News Detector")
st.markdown("Enter a news article below to check whether it is likely **Fake** or **Real**.")

model_choice = st.selectbox(
    "Select Model:",
    list(models.keys()),
    index=0,
    help="Choose a model: LR, DT, GBC, or RFC."
)
selected_model = models[model_choice]
...
st.markdown(
    """
    ---
    *Disclaimer: This detector uses machine learning models and may not be 100% accurate.*
    """
)
